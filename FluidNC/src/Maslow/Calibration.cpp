        ...
        case CALIBRATION_IN_PROGRESS:  //We can enter calibration in progress from EXTENDEDOUT, READY_TO_CUT, or CALIBRATION_COMPUTING
            if (currentState == EXTENDEDOUT || currentState == READY_TO_CUT || currentState == CALIBRATION_COMPUTING) {
                ...
                if (waypoint == 6 && pointCount == 6) {
                    ...
                    // Save the calibration data AND grid coordinates from the first 6 waypoints before deallocation
                    ...
                    // Restore the calibration data AND grid coordinates from the first 6 waypoints
                    for (int i = 0; i < 6; i++) {
                        for (int j = 0; j < 4; j++) {
                    ...
                    log_debug("About to log waypoint coordinates, pointCount=" << pointCount << ", allocatedPoints=" << allocatedPoints);
                    log_info("Waypoint coordinates:");
                    for (int i = 0; i < pointCount; i += 30) {
                        log_debug("Outer loop i=" << i << ", will process waypoints " << i << " to " << min(i + 29, pointCount - 1));
                        ...
                        for (int j = i; j <= min(i + 29, pointCount - 1); j++) {
                            log_debug("About to access calibrationGrid indices GRID_X(" << j << ")=" << GRID_X(j) << " and GRID_Y(" << j << ")=" << GRID_Y(j));
...
void Calibration::calibration_loop() {
    ...
    if (calibrationComplete) {  //Check if calibration is complete
        ...
        log_debug("Starting belt measurements loop, allocatedPoints=" << allocatedPoints);
        for (int i = 0; i < waypoint; i += 5) {
            log_debug("Outer loop iteration i=" << i << ", processing waypoints " << i << " to " << min(i + 4, waypoint - 1));
            // Build log message more carefully to avoid heap issues
            std::stringstream logMsg;
            logMsg << "Belt lengths [" << i << "-" << min(i + 4, waypoint - 1) << "]: ";
            for (int j = i; j < min(i + 5, waypoint); j++) {
                log_debug("About to access calibration_data[" << j << "]");
        ...
        log_debug("Finished belt measurements loop successfully");
