...
void MaslowKinematics::validateAndCorrectAnchorCoordinates() {
    ...
    // Check side lengths - use configurable min/max frame size limits
    const float MIN_SIDE_LENGTH = Maslow.frameSizeMin;
    const float MAX_SIDE_LENGTH = Maslow.frameSizeMax;
    ...
    if (topSideLength < MIN_SIDE_LENGTH || topSideLength > MAX_SIDE_LENGTH || rightSideLength < MIN_SIDE_LENGTH ||
        ...
        snprintf(buffer,
                 1400,
                 "Frame side lengths are outside valid range (%.0f-%.0fmm). "
