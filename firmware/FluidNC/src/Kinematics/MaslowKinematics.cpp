...
namespace Kinematics {
    ...
    void MaslowKinematics::calculateCenter() {
        ...
        float A  = (anchor_location[1][1] - anchor_location[2][1]) / (anchor_location[1][0] - anchor_location[2][0]);  // (trY - blY) / (trX - blX)
        float B  = (anchor_location[3][1] - anchor_location[0][1]) / (anchor_location[3][0] - anchor_location[0][0]);  // (brY - tlY) / (brX - tlX)
        _centerX = (anchor_location[3][1] - (B * anchor_location[3][0]) + (A * anchor_location[1][0]) - anchor_location[1][1]) / (A - B);
        _centerY = A * (_centerX - anchor_location[1][0]) + anchor_location[1][1];

        //log_info("Maslow center calculated: X=" << _centerX << " Y=" << _centerY);
    }

    // Generic getter for anchor coordinates
    float MaslowKinematics::getAnchorCoord(int arm, int axis) const {
        if (arm < 0 || arm >= 4 || axis < 0 || axis >= 3) {
            return 0.0f;
        }
        return anchor_location[arm][axis];
    }
    ...
    void MaslowKinematics::motors_to_cartesian(float* cartesian, float* motors, int n_axis) {
        ...
        */
        ...
        float z        = motors[4];
        float tlTotalZ = 0.0f - (z + anchor_location[0][2] + _spoilboardThickness + _workThickness);  // TL Z
        float trTotalZ = 0.0f - (z + anchor_location[1][2] + _spoilboardThickness + _workThickness);  // TR Z
    ...
    float MaslowKinematics::compute(int arm, float x, float y, float z) {
        ...
        x = x + _centerX;
        y = y + _centerY;
        float a = anchor_location[arm][0] - x;  // X dist from corner to router center
        float b = anchor_location[arm][1] - y;  // Y dist from corner to router center
        // When fixedZ is true, don't use current Z position - only use fixed anchor Z values
        float effectiveZ = _fixedZ ? 0.0f : z;
        float c          = 0.0f - (effectiveZ + anchor_location[arm][2] + _spoilboardThickness +
    ...
    bool MaslowKinematics::computeXYfromBeltLengths(float tlLength, float trLength, float& x, float& y) const {
        ...
        double d = sqrt((anchor_location[0][0] - anchor_location[1][0]) * (anchor_location[0][0] - anchor_location[1][0]) +
                        (anchor_location[0][1] - anchor_location[1][1]) * (anchor_location[0][1] - anchor_location[1][1]));
        ...
        double x0   = anchor_location[0][0] + a * (anchor_location[1][0] - anchor_location[0][0]) / d;  // TL X + ...
        double y0   = anchor_location[0][1] + a * (anchor_location[1][1] - anchor_location[0][1]) / d;  // TL Y + ...
        double rawX = x0 + h * (anchor_location[1][1] - anchor_location[0][1]) / d;  // TR Y - TL Y
        double rawY = y0 - h * (anchor_location[1][0] - anchor_location[0][0]) / d;  // TR X - TL X
    ...
    void MaslowKinematics::group(Configuration::HandlerBase& handler) {
        handler.item("tlX", anchor_location[0][0]);
        handler.item("tlY", anchor_location[0][1]);
        handler.item("tlZ", anchor_location[0][2]);
        handler.item("trX", anchor_location[1][0]);
        handler.item("trY", anchor_location[1][1]);
        handler.item("trZ", anchor_location[1][2]);
        handler.item("blX", anchor_location[2][0]);
        handler.item("blY", anchor_location[2][1]);
        handler.item("blZ", anchor_location[2][2]);
        handler.item("brX", anchor_location[3][0]);
        handler.item("brY", anchor_location[3][1]);
        handler.item("brZ", anchor_location[3][2]);
    ...
    void MaslowKinematics::setFrameSize(float frameSize) {
        ...
        anchor_location[2][0] = 0.0f;       // BL X
        anchor_location[2][1] = 0.0f;       // BL Y
        anchor_location[3][0] = frameSize;  // BR X
        anchor_location[3][1] = 0.0f;       // BR Y
        anchor_location[0][0] = 0.0f;       // TL X
        anchor_location[0][1] = frameSize;  // TL Y
        anchor_location[1][0] = frameSize;  // TR X
        anchor_location[1][1] = frameSize;  // TR Y
    ...
    void MaslowKinematics::validateAndCorrectAnchorCoordinates() {
        ...
        if (std::abs(anchor_location[2][0]) > TOLERANCE) {
            log_warn("Bottom left X coordinate (blX) should be 0.0, but is " << anchor_location[2][0] << ". Correcting to 0.0.");
            anchor_location[2][0]                 = 0.0f;
            coordinatesCorrected = true;
        }

        if (std::abs(anchor_location[2][1]) > TOLERANCE) {
            log_warn("Bottom left Y coordinate (blY) should be 0.0, but is " << anchor_location[2][1] << ". Correcting to 0.0.");
            anchor_location[2][1]                 = 0.0f;
            coordinatesCorrected = true;
        }

        if (std::abs(anchor_location[3][1]) > TOLERANCE) {
            log_warn("Bottom right Y coordinate (brY) should be 0.0, but is " << anchor_location[3][1] << ". Correcting to 0.0.");
            anchor_location[3][1]                 = 0.0f;
            coordinatesCorrected = true;
        }

        // Check that tlX < trX (left should be to the left of right)
        if (anchor_location[0][0] >= anchor_location[1][0]) {
            log_warn("Top left X coordinate (tlX=" << anchor_location[0][0] << ") should be less than top right X coordinate (trX=" << anchor_location[1][0]
                                                   << "). Correcting to reasonable defaults.");
            anchor_location[0][0]                 = DEFAULT_TLX;
            anchor_location[1][0]                 = DEFAULT_TRX;
            coordinatesCorrected = true;
        }

        // Check that top points are above bottom points
        if (anchor_location[0][1] <= anchor_location[2][1] || anchor_location[1][1] <= anchor_location[3][1]) {
            ...
            log_warn(buffer);
            releaseLogBuffer();
            anchor_location[0][1]                 = DEFAULT_TLY;
            anchor_location[1][1]                 = DEFAULT_TRY;
            coordinatesCorrected = true;
        }
        ...
        float topSideLength    = sqrt((anchor_location[1][0] - anchor_location[0][0]) * (anchor_location[1][0] - anchor_location[0][0]) + (anchor_location[1][1] - anchor_location[0][1]) * (anchor_location[1][1] - anchor_location[0][1]));
        float rightSideLength  = sqrt((anchor_location[3][0] - anchor_location[1][0]) * (anchor_location[3][0] - anchor_location[1][0]) + (anchor_location[3][1] - anchor_location[1][1]) * (anchor_location[3][1] - anchor_location[1][1]));
        float bottomSideLength = sqrt((anchor_location[3][0] - anchor_location[2][0]) * (anchor_location[3][0] - anchor_location[2][0]) + (anchor_location[3][1] - anchor_location[2][1]) * (anchor_location[3][1] - anchor_location[2][1]));
        float leftSideLength   = sqrt((anchor_location[0][0] - anchor_location[2][0]) * (anchor_location[0][0] - anchor_location[2][0]) + (anchor_location[0][1] - anchor_location[2][1]) * (anchor_location[0][1] - anchor_location[2][1]));
        ...
        if (anchor_location[0][1] < 0 || anchor_location[1][1] < 0 || anchor_location[0][1] > MAX_REASONABLE_COORD || anchor_location[1][1] > MAX_REASONABLE_COORD || anchor_location[2][0] < 0 || anchor_location[3][0] < 0 ||
            anchor_location[3][0] > MAX_REASONABLE_COORD) {
            log_warn("Anchor coordinates contain unrealistic values. Resetting to reasonable defaults.");
            anchor_location[0][0]                 = DEFAULT_TLX;
            anchor_location[0][1]                 = DEFAULT_TLY;
            anchor_location[1][0]                 = DEFAULT_TRX;
            anchor_location[1][1]                 = DEFAULT_TRY;
            anchor_location[2][0]                 = DEFAULT_BLX;
            anchor_location[2][1]                 = DEFAULT_BLY;
            anchor_location[3][0]                 = DEFAULT_BRX;
            anchor_location[3][1]                 = DEFAULT_BRY;
            coordinatesCorrected = true;
        }
