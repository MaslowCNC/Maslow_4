...
static int parse_motor_label(const char* motorLabel) {
    if (!motorLabel || !*motorLabel) {
        return _TL;
    }
    ...
    for (const char* p = motorLabel; *p && idx < sizeof(normalized) - 1; p++) {
        ...
        normalized[idx++] = toupper(static_cast<unsigned char>(*p));
    }
    normalized[idx] = '\0';

    if (strcmp(normalized, "TL") == 0 || strcmp(normalized, "TOPLEFT") == 0) {
        return _TL;
    }
    if (strcmp(normalized, "TR") == 0 || strcmp(normalized, "TOPRIGHT") == 0) {
        return _TR;
    }
    if (strcmp(normalized, "BL") == 0 || strcmp(normalized, "BOTTOMLEFT") == 0) {
        return _BL;
    }
    if (strcmp(normalized, "BR") == 0 || strcmp(normalized, "BOTTOMRIGHT") == 0) {
        return _BR;
    }
    return -1;
}

static const char* motor_code_for_axis(int axisId) {
    switch (axisId) {
        case _TL:
            return "TL";
        case _TR:
...
void Maslow_::getMotorInfo(const char* motorLabel) {
    const int motorAxis = parse_motor_label(motorLabel);
    if (motorAxis < 0 || motorAxis >= ARM_COUNT) {
        log_error("MOTORINFO invalid motor '" << (motorLabel ? motorLabel : "") << "', expected TL, TR, BL, or BR");
        return;
    }
...
bool Maslow_::setMotorTestSpeed(const char* motorLabel, int pwmSpeed) {
    const int motorAxis = parse_motor_label(motorLabel);
    if (motorAxis < 0 || motorAxis >= ARM_COUNT) {
    ...
    pwmSpeed = constrain(pwmSpeed, -1023, 1023);

    if (pwmSpeed == 0) {
        axis[motorAxis].stop();
    } else {
        axis[motorAxis].runAtSpeed(pwmSpeed);
    }

    log_info("MOTORTEST " << motor_code_for_axis(motorAxis) << " PWM " << pwmSpeed);
    return true;
}
