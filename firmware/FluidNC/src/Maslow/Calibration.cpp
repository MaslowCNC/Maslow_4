    ...
    switch (newState) {
        ...
        case CALIBRATION_IN_PROGRESS:  //We can enter calibration in progress from EXTENDEDOUT, READY_TO_CUT, or CALIBRATION_COMPUTING
            if (currentState == EXTENDEDOUT || currentState == READY_TO_CUT || currentState == CALIBRATION_COMPUTING) {
                ...
                int transitioningFrom = currentState;
                ...
                if (transitioningFrom != READY_TO_CUT) {
                    log_info("CALIBRATION_IN_PROGRESS: Setting FluidNC to Homing (entry point 1). Previous Maslow state: " << stateNames[transitioningFrom - 1].name);
                    sys.set_state(State::Homing);
                } else {
                ...
                // Only keep FluidNC in Idle if we're at waypoint 0, FluidNC is Idle, AND we're transitioning from READY_TO_CUT
                // In all other cases (including starting calibration from EXTENDEDOUT), set FluidNC to Homing
                if (waypoint == 0 && sys.state() == State::Idle && currentState == READY_TO_CUT) {
                    log_info("CALIBRATION_IN_PROGRESS: Keeping FluidNC in Idle (entry point 2, transitioning from READY_TO_CUT at waypoint 0)");
                } else {
        ...
        case READY_TO_CUT:  //We can enter ready to cut from calibration in progress, calibration computing or taking slack
            if (currentState == CALIBRATION_IN_PROGRESS || currentState == CALIBRATION_COMPUTING || currentState == TAKING_SLACK) {
                ...
                // Per requirements: Always transition to Idle to ensure machine is ready for operation
                State currentFluidNCState = sys.state();
                log_info("Maslow entering READY_TO_CUT state. FluidNC state was: " << stateName(currentFluidNCState));
                sys.set_state(State::Idle);
                log_info("FluidNC state set to Idle");
